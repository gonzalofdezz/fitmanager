...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    public JwtAuthenticationFilter(JwtService jwtService, UsuarioRepositoryOutputPort usuarioRepository) {
    ...
    protected void doFilterInternal(
        ...
        // Dejar pasar preflight OPTIONS sin validar token
        if ("OPTIONS".equalsIgnoreCase(request.getMethod())) {
            filterChain.doFilter(request, response);
            return;
        }

        // Dejar pasar /auth/** sin token (login/registro)
        ...
        if (email != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            ...
            if (usuarioOpt.isPresent() && jwtService.isTokenValid(jwt, email)) {
                ...
                String rol = usuario.rol() != null ? usuario.rol().name() : "USER";
