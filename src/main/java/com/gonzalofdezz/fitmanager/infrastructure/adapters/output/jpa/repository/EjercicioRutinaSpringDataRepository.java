package com.gonzalofdezz.fitmanager.infrastructure.adapters.output.jpa.repository;

import com.gonzalofdezz.fitmanager.infrastructure.adapters.output.jpa.data.EjercicioRutinaJpaEntity;
import org.springframework.data.jpa.repository.JpaRepository;
import org.springframework.stereotype.Repository;

import java.util.List;
import java.util.UUID;

@Repository
public interface EjercicioRutinaSpringDataRepository extends JpaRepository<EjercicioRutinaJpaEntity, UUID> {
    List<EjercicioRutinaJpaEntity> findByRutinaId(UUID rutinaId);
    void deleteByRutinaId(UUID rutinaId);
}

